...
export class Migration20240730015648 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "user" ("id" serial primary key, "created_at" timestamptz not null, "updated_at" timestamptz not null, "name" text not null);');

    this.addSql('create table "book" ("id" serial primary key, "created_at" timestamptz not null, "updated_at" timestamptz not null, "title" text not null, "description" varchar(1000) not null, "content" text not null, "author_id" int null);');
    ...
    this.addSql('alter table "book" add constraint "book_author_id_foreign" foreign key ("author_id") references "user" ("id") on update cascade on delete set null;');
